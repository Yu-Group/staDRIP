...
for (type in c("mirna", "methyl", "rnaseq", "prot")) {
  Xvalid <- as.matrix(all.data.valid[[type]])
  kernel.fit <- readRDS(paste0(save.path, type, "_kernel_fits.rds"))[[5]]
...
get_stab_scores <- function(drug, boot = 100){
    ...
    protein.cv.model = rep(0, ncol(protein))
    protein.kernel.model = rep(0, ncol(protein))
    protein.rf.model = rep(0, ncol(protein))
    rnaseq.escv.model = rep(0, ncol(rnaseq))
    rnaseq.kernel.model = rep(0, ncol(rnaseq))
    rnaseq.cv.model = rep(0, ncol(rnaseq))
    ...
    for(i in 1:boot){
        ...
        protein.cv.model = protein.cv.model + (coef(protein.escv$glmnet.fit, s = protein.escv$lambda.cv)[-1] !=0)
        ...
        rnaseq.cv.model = rnaseq.cv.model + (coef(rnaseq.escv$glmnet.fit, s = rnaseq.escv$lambda.cv)[-1] !=0)
        #thresh_rf = mean(ra$variable.importance) + 2*sqrt(var(ra$variable.importance))
    ...
    return(list(rnaseq.escv = rnaseq.escv.model,
                ...
                rnaseq.cv = rnaseq.cv.model,
                protein.escv = protein.escv.model,
                protein.rf = protein.rf.model,
                protein.cv = protein.cv.model))
}

scores = mclapply(1:24, get_stab_scores, mc.cores = 6)

saveRDS(scores, paste0(save.path, "stab_scores_sep.rds"))
```

```{r overlap-features}

scores = readRDS(paste0(save.path, "stab_scores_sep.rds"))
...
for (drug in 1:24){
    ...
    protein.cv.top = order(score[['protein.cv']], decreasing = TRUE)[1:10]
    protein.rf.top = order(score[['protein.rf']], decreasing = TRUE)[1:10]
    rnaseq.escv.top = order(score[['rnaseq.escv']], decreasing = TRUE)[1:10]
    rnaseq.cv.top = order(score[['rnaseq.cv']], decreasing = TRUE)[1:10]
    rnaseq.rf.top = order(score[['rnaseq.rf']], decreasing = TRUE)[1:10]
    protein.index = intersect(intersect(protein.escv.top, protein.rf.top), protein.cv.top)
    rnaseq.index = intersect(intersect(rnaseq.escv.top, rnaseq.cv.top), rnaseq.rf.top)
    ...
    stab.features[drug,] = c(as.character(paste(protein.features, collapse = ", ")), 
                             as.character(paste(rnaseq.features, collapse = ", ")))
}

myKable(stab.features,
        caption = "Stable protein and RNAseq signatures. A feature is included if it is among the top 10 most stable features under 3 different machine learning models (i.e., Lasso (CV), Lasso (ESCV), and random forest). The stability of the features are computed from the PCS inference framework. Blank cells indicate that no features appeared among the top 10 most stable features for all three models.")
...
We fit separate models to the two datasets due to the need to jointly interpret the RNASeq and protein expressions and the aforementioned limitations of the alternative concatenating approach. 
Next, for each feature $X_j$ from either the protein or RNAseq data set, let $\omega_j^{(b)}$ be defined in the following way: for the Lasso, $\omega_j^{(b)}=1$ if the coefficient of $X_j$ is non-zero, and $\omega_j^{(b)}=0$ otherwise; for the random forest, $\omega_j^{(b)}$ is the MDI feature importance of $X_j$.
